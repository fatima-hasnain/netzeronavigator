...
5. **Outputs** — [`tensorOutputFeatures`](../src/lib/tfFeatureSelection.ts): `kind === "surrogate-output"` with `tf`, sorted by `tf.position`. Each raw output dimension is de-standardized, then **`Math.exp`** is applied ([`rawPredictionToOutputs`](../src/tf/tensorPipeline.ts)), matching legacy `exponentiate` after `de-standardize`.
...
Reference (read-only): `net-zero-navigator-snapshot/subsystems/webapp/src/clj/nzn/browser/tfjs.cljs`.

## Exploration UI (dual pane)

On a surrogate route, [`useSurrogateExploration`](../src/hooks/useSurrogateExploration.ts) runs **debounced** predictions (default **150 ms**) whenever tensor **input values** change. Only the result of the **latest** completed run is applied (sequence id), so rapid slider changes do not show stale numbers.
...
- **Output pane** — [`OutputReadoutPane`](../src/components/OutputReadoutPane.tsx): shows tensor outputs after de-standardize + `exp`, with an “Updating…” hint while a run is in progress.
...
Surrogate folders live under `public/models/<surrogateId>/`. For deployment behind a CDN, set `VITE_MODELS_BASE` to the public base URL (no trailing slash). See `.env.example`.
